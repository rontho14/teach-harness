---
name: teach
description: Teach the user in this workspace. Context-aware. In an ORGANIZER folder it runs intake for a new subject (interview the mission, pick language, research resources, draft a plan). In a CLASSROOM (a folder with MISSION.md) it produces the next artifact in the learner's zone of proximal development — a lesson + exercise, a cumulative review every 5th lesson, an exam every 10th, or a long-running personal project (a real-world deliverable, on request or at a mission milestone) — and keeps the FSRS schedule current. Use whenever the user wants to learn, continue a course, or be taught.
...
counters (`lessons_taught`, `reviews_done`, `exams_done`, `projects_done`,
`last_session_date`). Let **N** = `lessons_taught` (new-material lessons only;
reviews, exams, and projects have their own counters and never move N). As a sanity
check, N should equal the number of files in `lessons/`; if it doesn't, trust
`course.json` but flag the drift.
...
**Personal projects run on a separate, parallel track** — they are NOT part of the
numeric decision above. Start or advance one (see 3d) when the learner **asks**
("start a project", "I want to build X"), or **propose** one when a `MISSION.md`
success gate becomes reachable. A project never blocks lesson N+1 and never moves
the counters above; if a project is `active`, surface its next check-in but keep
producing lessons/reviews/exams as normal.
...
### 3d. Produce / advance a PROJECT
Follow `.claude/rules/PROJECT-DESIGN.md`. A **personal project** is a long-running,
real-world deliverable tied to a `MISSION.md` success gate (e.g. ship a repo,
build & defend your own training program, run a marathon). It runs **in parallel**
with lessons — never gate lesson N+1 on it.
- **Starting one:** research the deliverable's real-world shape first (and cite),
  then create `projects/NNNN-slug/` with `brief.html` from
  `.claude/templates/project.html` (goal, deliverable, **acceptance gates** with
  committed `RUBRICS`, exercised sub-concepts, check-in cadence) and a `log.md`
  with status `proposed`/`active`.
- **Advancing one:** when the learner submits a check-in (graded like any artifact,
  see Grading), append it to `log.md`. Mark status `delivered` and bump
  `projects_done` only when all acceptance gates are met.
...
- **Bump `course.json`** for the artifact you just produced: a **lesson** →
  ...
  an **exam** → `exams_done += 1`; a **delivered project** → `projects_done += 1`
  (only on final delivery, not on creation or a mid-project check-in). Always set
  `last_session_date` to today. This keeps the 5/10 milestone math off the
  filesystem so renaming a file never shifts the rhythm.
